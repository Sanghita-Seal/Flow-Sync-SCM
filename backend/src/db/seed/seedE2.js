...
function readSheet(sheetName) {
  ...
  const actualSheetName = workbook.SheetNames.find(
    (name) => name.toLowerCase().trim() === sheetName.toLowerCase().trim(),
  );
    ...
    for (const row of docks) {
      ...
      await client.query(
        `
                ...
                `,
        [row.dock_code, row.yard_name, row.status || "AVAILABLE"],
      );
    }
    ...
    for (const row of shipments) {
      await client.query(
        ...
        [
          ...
          row.planned_arrival || null,
          row.planned_quantity_m ?? null,
          row.received_quantity_m ?? null,
        ],
      );
    }
    ...
    for (const row of dockAssignments) {
      ...
      await client.query(
        `
                ...
                `,
        [row.trailer_id, row.dock_code, row.yard_name],
      );
    }
    ...
    for (const row of truckAlerts) {
      ...
      await client.query(
        `
                ...
                `,
        [row.trailer_id, row.alert_reason, row.message],
      );
    }
