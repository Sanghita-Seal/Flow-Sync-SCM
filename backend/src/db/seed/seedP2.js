...
async function seedP2() {
  ...
  try {
    ...
    // ==========================================
    // RESET P2 DATA SAFELY
    // ==========================================
    // IMPORTANT:
    // Do NOT use TRUNCATE ... CASCADE here.
    // e2.shipments.procurement_plan_id will reference
    // p2.procurement_plans.id.
    // DELETE allows ON DELETE SET NULL to preserve E2 shipments.

    await client.query(`DELETE FROM p2.sop_recommendations`);
    await client.query(`DELETE FROM p2.procurement_plans`);
    await client.query(`DELETE FROM p2.sop_plan_lines`);

    await client.query(`DELETE FROM p2.markdown_history`);
    await client.query(`DELETE FROM p2.logistics_routes`);
    await client.query(`DELETE FROM p2.production_capacity`);
    await client.query(`DELETE FROM p2.inventory`);
    await client.query(`DELETE FROM p2.sell_through`);
    await client.query(`DELETE FROM p2.demand_forecasts`);

    await client.query(`DELETE FROM p2.sop_cycles`);

    await client.query(`DELETE FROM p2.products`);
    await client.query(`DELETE FROM p2.fabrics`);
    await client.query(`DELETE FROM p2.plants`);
    await client.query(`DELETE FROM p2.suppliers`);
    ...
    for (const supplier of suppliers) {
      ...
      supplierMap.set(supplier.supplier_code, result.rows[0].id);
    }
    ...
    for (const plant of plants) {
      ...
      plantMap.set(plant.plant_code, result.rows[0].id);
    }
    ...
    for (const fabric of fabrics) {
      const supplierId = supplierMap.get(fabric.supplier_id);

      if (!supplierId) {
        throw new Error(`Supplier not found: ${fabric.supplier_id}`);
      }
      ...
      fabricMap.set(fabric.fabric_code, result.rows[0].id);
    }
    ...
    for (const product of products) {
      const fabricId = fabricMap.get(product.fabric_id);

      const plantId = plantMap.get(product.plant_id);

      if (!fabricId) {
        throw new Error(`Fabric not found: ${product.fabric_id}`);
      }

      if (!plantId) {
        throw new Error(`Plant not found: ${product.plant_id}`);
      }
      ...
      productMap.set(product.sku_code, result.rows[0].id);
    }
      ...
      await client.query(
        `
                ...
                VALUES
                    ($1, $2, $3, $4)
                `,
        [productId, row.week, row.week_number, row.forecast_demand_units],
      );
    }
      ...
      await client.query(
        `
                ...
                VALUES
                    ($1, $2)
                `,
        [productId, row.current_inventory_units],
      );
    }
      ...
      await client.query(
        `
                ...
                VALUES
                    ($1, $2, $3, $4, $5)
                `,
        [plantId, productId, row.week, row.week_number, row.capacity_units],
      );
    }
      ...
      await client.query(
        `
                ...
                VALUES
                    ($1, $2, $3, $4, $5)
                `,
        [productId, row.week, row.week_number, row.markdown_pct, row.reason],
      );
    }
    ...
    const cycleResult = await client.query(
      `
            ...
            RETURNING id
            `,
      ["SOP-2026-AUG", "2026-08-01", "2026-08-31", "DRAFT"],
    );
